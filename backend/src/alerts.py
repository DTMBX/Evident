...
class AlertManager:
    ...
    @staticmethod
    def _send_alert(user: User, alert_type: str, quota_name: str, percent: float, quota_obj: UsageQuota):
        """
    ...
    @staticmethod
    def _build_alert_email(user: User, alert_type: str, quota_name: str, percent: float, quota: UsageQuota) -> str:
    ...
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="alert-box {'warning' if percent < 95 else 'critical'}">
                <h3>{quota_name}</h3>
                <p style="font-size: 24px; margin: 10px 0;"><strong>{percent:.1f}%</strong> of quota used</p>
            </div>
            
            <div class="stats">
                <h4>Your Current Usage:</h4>
                <ul>
                    <li>AI Tokens: {quota.ai_tokens_used:,} / {quota.ai_tokens_limit:,} ({quota.get_usage_percent('ai_tokens'):.1f}%)</li>
                    <li>AI Requests: {quota.ai_requests_count} / {quota.ai_requests_limit} ({quota.get_usage_percent('ai_requests'):.1f}%)</li>
                    <li>Storage: {quota.storage_bytes_used / 1048576:.1f} MB / {quota.storage_bytes_limit / 1048576:.1f} MB ({quota.get_usage_percent('storage'):.1f}%)</li>
                    <li>Files: {quota.files_uploaded_count} / {quota.files_uploaded_limit} ({quota.get_usage_percent('files'):.1f}%)</li>
                    <li>Cost: ${float(quota.total_cost_usd):.2f} / ${float(quota.cost_limit_usd):.2f} ({quota.get_usage_percent('cost'):.1f}%)</li>
                </ul>
            </div>
            ...
            <p style="text-align: center; margin: 30px 0;">
                <a href="https://Evident/pricing" class="button">View Upgrade Options</a>
            </p>
            
            <p style="text-align: center; margin: 30px 0;">
                <a href="https://Evident/workspace">View Your Dashboard</a>
            </p>
        </div>
...
"""
    ...
    def check_all_users_quotas():
        ...
        current_app.logger.info(f"Quota check complete. {total_alerts} alerts sent across {len(users)} users.")
        return total_alerts
