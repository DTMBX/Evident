...
function scan(){
  ...
  const report = { scannedAt: new Date().toISOString(), pages: {}, brokenLinks: [], missingAssets: [] };
  ...
  for (const file of htmlFiles) {
    ...
    const sources = [];
    $('source').each((i, el) => sources.push($(el).attr('src') || $(el).attr('data-src') || $(el).attr('data-src-mp4-1080')));
    ...
    for (const ref of allRefs) {
      ...
      if (resolved.type === 'internal') {
        // allow directory index (/) mapping to index.html
    ...
    // check images/scripts/links existence for local files
  ...
  const out = path.join(outDir, 'site-scan.json');
  fs.writeFileSync(out, JSON.stringify(report, null, 2));
  console.log('Scan complete — report written to', out);
