...
def main():
    ...
    success, sp_module = test_import(pipeline_path, "SmartPipelineSelector", "SmartPipelineSelector")
    if success:
        results["passed"] += 1
    else:
        results["failed"] += 1
    ...
    try:
        ...
        result = subprocess.run(
            [sys.executable, "bwc_chunk_analyzer.py"],
            capture_output=True,
            text=True,
            timeout=30
        )
    ...
    try:
        os.chdir(base_path)
        with open(base_path / "bwc_api_routes.py", 'r') as f:
            api_code = f.read()
    ...
    try:
        with open(base_path / "_includes" / "bwc-chunk-analyzer.html", 'r') as f:
            ui_code = f.read()
    ...
    try:
        with open(base_path / "assets" / "js" / "bwc-chunk-ui.js", 'r') as f:
            js_code = f.read()
    ...
    for doc_path, keywords in docs_to_check:
        try:
            with open(base_path / doc_path, 'r') as f:
                content = f.read().lower()
            
            found_keywords = sum(1 for kw in keywords if kw in content)
            if found_keywords == len(keywords):
                print_test(f"{Path(doc_path).name}: Complete ({found_keywords}/{len(keywords)} keywords)", "success")
                results["passed"] += 1
            else:
                print_test(f"{Path(doc_path).name}: Incomplete ({found_keywords}/{len(keywords)} keywords)", "warning")
                results["warnings"] += 1
        except Exception as e:
    ...
    integration_checks = [
        ("Check if bwc_forensic_analyzer.py exists", Path(base_path / "bwc_forensic_analyzer.py").exists()),
