...
def test_enqueue_and_run_job(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(sync, "extract_audio_wav", lambda p, out=None: str(Path(p).with_suffix('.wav')))
    monkeypatch.setattr(sync, "build_sync_map", lambda paths, reference_index=0: {"entries": []})

    # Enqueue
    job_id = enqueue_batch(case_id="C1", user_id=42, upload_ids=[1, 2], video_paths=[str(v1), str(v2)], options={})
