...
class TestAuthenticationSecurity:
    ...
    def test_session_token_security(self, client):
        ...
        response = client.post(
            "/auth/login", json={"email": "test@test.com", "password": "testpass123"}
        )
        ...
        for cookie in cookies:
            if "session" in cookie.lower():
                ...
                assert (
                    "Secure" in cookie or "localhost" in cookie
    ...
    def test_rate_limiting_on_login(self, client):
        ...
        for i in range(10):
            response = client.post(
                "/auth/login", json={"email": "test@test.com", "password": f"wrongpass{i}"}
            )

        # After 10 attempts, should be rate limited
        response = client.post(
            "/auth/login", json={"email": "test@test.com", "password": "wrongpass11"}
        )
...
class TestAuthorizationSecurity:
    ...
    def test_tier_enforcement(self, client, app, db_session):
        ...
        client.post("/auth/login", json={"email": "starter@test.com", "password": "testpass123"})
    ...
    def test_direct_tier_upgrade_blocked(self, client, app, db_session):
        ...
        # Login as FREE user
        client.post("/auth/login", json={"email": "free@test.com", "password": "testpass123"})

        # Try to directly modify tier
        response = client.post("/api/user/profile", json={"tier": "PROFESSIONAL"})
...
class TestSQLInjectionProtection:
    ...
    def test_login_sql_injection(self, client):
        ...
        for payload in sql_payloads:
            response = client.post("/auth/login", json={"email": payload, "password": "anything"})
            ...
            assert response.status_code in [
                ...
                401,
                422,
            ], f"SQL injection should be blocked: {payload}"
    ...
    def test_search_sql_injection(self, client, app, db_session):
        ...
        # Login
        client.post("/auth/login", json={"email": "pro@test.com", "password": "testpass123"})

        # Try SQL injection in search
        response = client.post("/api/legal-library/search", json={"query": "test' OR '1'='1"})
...
class TestXSSProtection:
    ...
    def test_stored_xss_in_case_notes(self, client, app, db_session):
        ...
        # Login
        client.post("/auth/login", json={"email": "xss@test.com", "password": "testpass123"})
        ...
        for payload in xss_payloads:
            response = client.post("/api/cases/notes", json={"case_id": 1, "note": payload})
...
class TestCSRFProtection:
    ...
    def test_csrf_token_required(self, client, app, db_session):
        ...
        # Login
        client.post("/auth/login", json={"email": "csrf@test.com", "password": "testpass123"})

        # Try to upload without CSRF token
        response = client.post(
            "/api/upload/video", json={"file": "test.mp4"}, headers={"X-Requested-With": ""}
        )  # No CSRF token
...
class TestAPISecurityHeaders:
    ...
    def test_api_cors_configuration(self, client):
        ...
        response = client.options("/api/health", headers={"Origin": "https://evil.com"})
...
class TestRateLimitingAndDDoSProtection:
    ...
    def test_api_rate_limiting(self, client, app, db_session):
        ...
        # Login
        client.post("/auth/login", json={"email": "ratelimit@test.com", "password": "testpass123"})
...
class TestDataEncryption:
    ...
    def test_sensitive_data_not_exposed(self, client, app, db_session):
        ...
        # Login
        response = client.post(
            "/auth/login", json={"email": "sensitive@test.com", "password": "SecurePass123!"}
        )
...
class TestAPIKeyManagement:
    ...
    def test_api_key_generation(self, client, app, db_session):
        ...
        # Login
        client.post("/auth/login", json={"email": "apikey@test.com", "password": "testpass123"})
    ...
    def test_api_key_revocation(self, client, app, db_session):
        ...
        # Login
        client.post("/auth/login", json={"email": "revoke@test.com", "password": "testpass123"})

        # Revoke API key
        response = client.post("/api/keys/revoke", json={"key_id": "test_key_123"})
