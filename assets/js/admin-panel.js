...
function confirmDeleteAnalysis(analysisId, filename) {
    if (confirm(`Are you sure you want to delete analysis "${filename}"? This action cannot be undone.`)) {
    ...
    for (const [category, settings] of Object.entries(settingsByCategory)) {
        ...
        for (const setting of settings) {
            ...
            const inputType = setting.value_type === 'bool' ? 'checkbox' : 
                             setting.value_type === 'int' || setting.value_type === 'float' ? 'number' : 'text';
            const inputValue = setting.value_type === 'bool' ? setting.value.toLowerCase() === 'true' : setting.value;
...
async function initializeSettings() {
    if (!confirm('This will create all default settings. Existing settings will not be modified. Continue?')) {
        return;
    }
    ...
    try {
        ...
        if (response.ok) {
            showToast(`Settings initialized: ${data.created} created, ${data.skipped} skipped`, 'success');
            await loadSettings();
        } else {
...
async function loadSystemInfo() {
    ...
    try {
        ...
        document.getElementById('uploadStorage').textContent = data.upload_storage_gb.toFixed(2) + ' GB';
...
async function loadAuditLogs() {
    ...
    try {
        ...
        const url = action ? `/admin/audit-logs?action=${action}&limit=200` : '/admin/audit-logs?limit=200';
